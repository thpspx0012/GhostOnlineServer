...
namespace Server.Common.IO.Packet
{
    public class OutPacket : PacketBase, IDisposable
    {
        ...
        public void WriteString(string value)
        {
            ...
            this.WriteShort((short)Encoding.GetEncoding("utf-8").GetBytes(value).Length);

            foreach (byte c in Encoding.GetEncoding("utf-8").GetBytes(value))
            {
        ...
        {
            byte[] ret = Encoding.GetEncoding("utf-8").GetBytes(value);
